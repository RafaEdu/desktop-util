      ...
      {fetched && certs.length > 0 && (
        <div className="px-4 pb-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-fg-5" />
            <input
              type="text"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Buscar por nome ou emissor..."
              className="w-full bg-field border border-edge-2 rounded-lg pl-9 pr-3 py-2 text-sm
                ...
                >
                  <div className="flex items-start gap-2 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-fg-5 truncate mt-0.5">
                        Emissor: {cert.issuer}
                      </p>
                      <p
                        className={cn(
                      ...
                      <p className="text-xs text-fg-6 mt-0.5 font-mono truncate">
                        {cert.thumbprint}
                      </p>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="p-2 text-fg-6 hover:text-red-400 hover:bg-red-900/20 rounded-md transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"
      ...
      */}
      ...
      {showBatchDeleteModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface border border-edge-2 rounded-xl shadow-2xl max-w-sm w-full p-4 animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-start mb-3">
              <div className="flex items-center gap-2 text-red-400">
                <Trash2 className="w-5 h-5" />
                <h3 className="font-semibold text-fg">Excluir Vencidos?</h3>
              </div>
              <button
                onClick={() => setShowBatchDeleteModal(false)}
                className="text-fg-5 hover:text-fg-3"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <details className="mb-4 text-xs group">
              <summary className="text-fg-5 cursor-pointer hover:text-indigo-400 transition-colors list-none flex items-center gap-1 select-none">
                <span className="group-open:hidden">▶ Ver lista</span>
                <span className="hidden group-open:inline">
                  ▼ Ocultar lista
                </span>
              </summary>
              <div className="mt-2 max-h-32 overflow-y-auto bg-base/50 rounded border border-edge p-2 space-y-1">
                {expiredCerts.map((c) => (
                  <div
                    key={c.thumbprint}
                    className="truncate text-fg-4 border-b border-edge/50 last:border-0 pb-1 last:pb-0"
                  >
                    {c.subject}
                  </div>
                ))}
            ...
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setShowBatchDeleteModal(false)}
                className="px-3 py-1.5 rounded-lg text-xs font-medium bg-field text-fg-3 border border-edge-2 hover:bg-subtle transition-colors"
              >
                Cancelar
              </button>
              <button
      ...
      {certToDelete && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface border border-edge-2 rounded-xl shadow-2xl max-w-sm w-full p-4 animate-in fade-in zoom-in duration-200">
            ...
            <div className="text-sm text-fg-3 mb-6">
              <p>Tem certeza que deseja remover o certificado abaixo?</p>
              <div className="mt-3 p-2 bg-base rounded border border-edge">
                <p className="font-medium text-fg-2 truncate">
